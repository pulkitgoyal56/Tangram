...
class Tangram(PolygonList, gym.Env):
    ...
    """Gym environment for Tangram (2D)."""
    def __init__(
        self,
        *,
    ...
    def observation_space(self):
        ...
        # return spaces.Box(low=-np.inf, high=np.inf, shape=(self.num_objects * self.num_actions + (2 if self.object_persistency else 0),), dtype=np.float32)
        return spaces.Box(low=-np.inf, high=np.inf, shape=(self.n * self.dim + (2 if self.object_persistency else 0),), dtype=np.float32)
    ...
    def render_fast(self, size=(224, 224), *, sift=True, color=True, lineType=8, **kwargs):
        if color:
            ...
            fillPoly(canvas, [[0, size[1] - 1] + (polygon.vertices * [size[0], -size[1]]).astype(int) for polygon in self.polygons if not sift or polygon in self], color=0, lineType=lineType)  # **kwargs
        return canvas
    ...
    def highlight_active(self, image: np.ndarray, v: list, *, boundaries: bool = False, vertices: bool = False, size: int = 5):
        ...
        def highlight(image: np.ndarray, xy: np.ndarray, *, scale: float = 1.):
            return Im.highlight(
                image,
                *Grid.xy2rc(xy - (not boundaries) * self.staging_boundaries[0], shape=image.shape[:2], size=(1., 1.) if boundaries else np.diff(self.staging_boundaries, axis=0)[0]),
                size=1, v=v if vertices or image.ndim == 3 else [255 - c for c in v], radius=scale * self._scale * size / (1 if boundaries else np.sqrt(Grid(*self.staging_boundaries).area))
            )
    ...
    def valid_pos(self, vertices, obj_id, *, constrained=False):
        ...
        if constrained and self.max_dist >= 0:  # self._mask[obj_id]  # Only required for unlocked objects
            if np.any((self.objects_[obj_id][:self.dim] >= 0) & (np.abs(Polygon.get_pivot(vertices) - self.objects_[obj_id][:self.dim]) > (self.max_dist if self.max_dist < 1 else [self.max_dist / size if size > 1 else size / self.max_dist for size in [self.x_size, self.y_size]]))):
                return False
